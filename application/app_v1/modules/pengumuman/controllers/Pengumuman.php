...
class Pengumuman extends CI_Controller {
	...
	public function publish($id)
	{
		...
		if ($data->published==0) {
			...
			$hsl='<label class="label label-success">Publish</label><a href="'.base_url().'pengumuman/cetakpdf/'.$id.'" class="btn btn-mini btn-link" target="_blank">Cetak to Pdf</a>';
		}
	...
	public function getReview()
	{
		...
		$getmailKabak=$this->db->get_where('user_trakses',['level_id'=>8])->row();
		$input = [
			'pengirim' =>$this->session->userdata('email'),
			'penerima'=>$getmailKabak->email,
			'lvl'=>8,
			'pesan'=>'kabak menforward Pengumuman Ke Anda cek di dengan Status Review Pengumuman',
			'status'=>1,
	...
	public function saveReview()
	{
		...
		$getmailKabak=$this->db->get_where('user_trakses',['level_id'=>11])->row();
		$input = [
			'pengirim' =>$this->session->userdata('email'),
			'penerima'=>$getmailKabak->email,
			'lvl'=>11,
	...
	public function cekDataPengumuman()
	{
		...
		$data['title_page']="Cek Data Pengumuman";	
	...
	public function val_operator()
	{
		...
		$getmailKabak=$this->db->get_where('user_trakses',['level_id'=>10])->row();
		$input = [
			'pengirim' =>$this->session->userdata('email'),
			'penerima'=>$getmailKabak->email,
			'lvl'=>10,
			'pesan'=>'Operator Telah Menforward Pengumuman Baru Cek Untuk Validasi',
			'status'=>1,
			'tgl_notif'=>date('Y-m-d H:i:s')
		];
		$this->db->insert('notifikasi', $input);
	...
	public function save()
	{
		...
		if ($method=='add') {
			...
			$val_pengumuman = [
				'pengumuman_id' =>$insert_id,
				'kabak'=>0,
				'wadir'=>0 
			];
			$this->master->create('pengumuman_validasi',$val_pengumuman);
	...
	public function saveHslReview()
	{
		...
		{
			$getPengumuman            =$this->db->get_where('pengumuman', array('id_pengumuman' =>$id ))->row();

			$chat_id                  = -1001284274192;
		// $chat_id                  = 956713352;
		// $text                     ='<b>Tanggal : '.date('l').', '.date('d F Y').' </b>
		// <b>Perihal : Pengumuman Kegiatan</b><pre> No Pengumuman : '.$getPengumuman->no_pengumuman.'</pre><a href="'.base_url().'pengumuman/lihat/'.$id.'">Cek pengumuman</a>';
			$text='<pre>Tanggal : '.date('l, d F Y ').'
			perihal : Pengumuman Akademik
			Nomor : '.$getPengumuman->no_pengumuman.'</pre>

			<a href="'.base_url().'pengumuman/lihat/'.$id.'">Cek pengumuman</a>';
			$parse_mode               ='HTML';
		...
		public function cetakpdf($id){
			...
			$data['date']=date('d').' '.$arrNamaBulan[date('m')].' '.date('Y');
			$data['isi'] =$this->db->get_where('pengumuman',array('id_pengumuman' =>$id ))->row();
			$this->load->view('content/cetakpdf',$data);
