		...
		if ($this->form_validation->run() == FALSE) {
			$data = [
				...
				'errors'	=> [
					...
					'nama_dsn' => strip_tags(form_error('nama')),
			...
			$input = [
				...
				'nidn' 			=> $this->input->post('nidn', true),
				'email' 		=> $this->input->post('email', true),
				'nama_dsn' 			=> $this->input->post('nama', true),
				'jenis_k' => $this->input->post('jenis_k', true),
				'prodi_id' => $this->input->post('prodi', true),
				
			];
			if ($method === 'add') {
				$action = $this->master->create('m_dosen', $input);
				$this->output->set_content_type('application/json')->set_output(json_encode(['add'=>$input,'status'=>true]));
			} else if ($method === 'edit') {
				$id = $this->input->post('id_dsn', true);
				$action = $this->master->update('m_dosen','id_dosen', $id,$input);
				$this->output->set_content_type('application/json')->set_output(json_encode(['edit'=>$id,'status'=>true]));
			}
